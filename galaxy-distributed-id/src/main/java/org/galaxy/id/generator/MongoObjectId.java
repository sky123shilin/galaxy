...
public final class MongoObjectId implements Comparable<MongoObjectId>, Serializable {
    ...
    public static MongoObjectId get() {
    ...
    public MongoObjectId(Date date) {
        this(dateToTimestampSeconds(date), NEXT_COUNTER.getAndIncrement() & 16777215, false);
    }

    public MongoObjectId(Date date, int counter) {
        this(dateToTimestampSeconds(date), counter, true);
    }

    public MongoObjectId(int timestamp, int counter) {
        this(timestamp, counter, true);
    }

    private MongoObjectId(int timestamp, int counter, boolean checkCounter) {
        this(timestamp, RANDOM_VALUE1, RANDOM_VALUE2, counter, checkCounter);
    }

    private MongoObjectId(int timestamp, int randomValue1, short randomValue2, int counter, boolean checkCounter) {
    ...
    public MongoObjectId(String hexString) {
    ...
    MongoObjectId(int timestamp, int machineAndProcessIdentifier, int counter) {
        this(legacyToBytes(timestamp, machineAndProcessIdentifier, counter));
    }

    public MongoObjectId(ByteBuffer buffer) {
    ...
    private static byte[] legacyToBytes(int timestamp, int machineAndProcessIdentifier, int counter) {
        byte[] bytes = new byte[]{
                ...
                int1(timestamp),
                int0(timestamp),
                int3(machineAndProcessIdentifier),
                int2(machineAndProcessIdentifier),
                int1(machineAndProcessIdentifier),
                int0(machineAndProcessIdentifier),
                int3(counter),
                int2(counter),
                int1(counter),
                int0(counter)
    ...
    public boolean equals(Object o) {
        if (this == o) {
            ...
            MongoObjectId objectId = (MongoObjectId)o;
